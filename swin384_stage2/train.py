...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('../swin384_stage1/results/car_uncropped_resnet50_SM_512_0.1_0.5_0.15_32_model.pth'),strict=False)
    ...
    flops, params = profile(model, inputs=(torch.randn(1, 3, 384, 384).cuda(),))
    flops, params = clever_format([flops, params])
    print('# Model Params: {} FLOPs: {}'.format(params, flops))
    optimizer = Adam(model.parameters(), lr=3e-5)
    #lr_scheduler = MultiStepLR(optimizer, milestones=[int(0.6 * num_epochs), int(0.8 * num_epochs)], gamma=0.1)
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=19, eta_min=1e-6)
