...
model.load_state_dict(torch.load('results/car_uncropped_resnet50_SM_512_0.1_0.5_0.15_88_model.pth'),
                      strict=False)
model.to(device)
model.eval()
pred_list = []
...
with torch.no_grad():
    ...
    val_pd.sort_values("prediction", inplace=True, ascending=False)
    val_pd.to_csv('pseudo.csv', index=False)
