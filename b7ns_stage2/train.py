...
import shutil
from model_b7 import Model, set_bn_eval
...
def test1(net, recall_ids):
    ...
    with torch.no_grad():
        ...
        if data_name == 'isc':
            ...
            acc_list = recall(eval_dict['test']['features'], test_data_set.labels, recall_ids)
            # acc_list_ = metrics.silhouette_score(eval_dict['test']['features'].cpu(), test_data_set.labels,metric='cosine')
    ...
    # print("silhouette_score  {}".format(acc_list_))
    print(desc)
    return acc_list[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epochs', default=20, type=int, help='train epoch number')  # 50轮就差不多了
    ...
    model.load_state_dict(torch.load('../b7ns_stage1/results/car_uncropped_resnet50_SM_512_0.1_0.5_0.15_32_model.pth'),
                      strict=False)
    ...
    lr_scheduler = CosineAnnealingLR(optimizer, T_max=19, eta_min=1e-6)
